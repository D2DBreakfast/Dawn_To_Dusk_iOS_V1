...
@IBDesignable public class PinterestSegment: UIControl {
    ...
    public struct TitleElement: Equatable {
        public let title: PintrestTitle
        public let selectedImage: UIImage?
        public let normalImage: UIImage?

        public init(title: PintrestTitle, selectedImage: UIImage? = nil, normalImage: UIImage? = nil) {
            self.title = title
        ...
        public static func == (lhs: TitleElement, rhs: TitleElement) -> Bool {
            return lhs.title.id == rhs.title.id && lhs.selectedImage == rhs.selectedImage && lhs.normalImage == rhs.selectedImage
        }
    }
    ...
    public var titles: [PintrestTitle] {
        get {
    ...
    public convenience init(frame: CGRect, titles: [PintrestTitle]) {
        self.init(frame: frame, segmentStyle: PinterestSegmentStyle(), titles: titles)
    }

    public init(frame: CGRect, segmentStyle: PinterestSegmentStyle, titles: [PintrestTitle]) {
    ...
    private func reloadData(animated: Bool = true, sendAction: Bool = true) {
        ...
        for (index, item) in titleElements.enumerated() {
            ...
            let titleW = toToSize(item.title.title!) + titlePendingHorizontal * 2
            ...
            backLabel.tag = index
            backLabel.text = item.title.title!
            ...
            frontLabel.text = item.title.title!
